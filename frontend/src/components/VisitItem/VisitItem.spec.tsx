...
describe('Visit Item component', () => {
    it('renders the summary information of a food intake visit', () => {
        ...
        expect(screen.getByText(`${mockFoodIntakeProps.id}`)).toBeInTheDocument();
        expect(screen.getByText(Date.parse(mockFoodIntakeProps.timestamp))).toBeInTheDocument();
        expect(screen.getByText(parseEventType(mockFoodIntakeProps.event_type))).toBeInTheDocument();
    })
})